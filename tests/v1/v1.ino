 ...
 * - Ultrasonic Sensor (Pins 27, 26) pushing presence (true/false) to Firebase.
 * - LED 1 (Pin 12) controlled by /led variable in Firebase.
 * - LED 2 (Pin 14) mirrors the state of /relay1.
 **********************************************************************************/
...
void handleEvent(AceButton* button, uint8_t eventType, uint8_t /* buttonState */) {
  ...
  switch (id) {
    case SwitchPin1:
      ...
      digitalWrite(LED_PIN_14, newState ? HIGH : LOW); // Sync LED 14 (Active HIGH)
...
void readAndPushSensors() {
  ...
  float tempF = dht.readTemperature(true); // Read as Fahrenheit (matches '68' in screenshot)
...
void loop() {
  ...
  if (Firebase.ready()) {
    ...
    if (Firebase.RTDB.getBool(&fbdo, "/led")) {
      led12_state = fbdo.boolData();
      digitalWrite(LED_PIN_12, led12_state ? HIGH : LOW);
    }
